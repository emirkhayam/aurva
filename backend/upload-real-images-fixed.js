...
function generateUniqueFilename(originalName, fileExt) {
  ...
  return `news-${hash}-${timestamp}${fileExt}`;
}
...
async function uploadImageToStorage(imagePath, slug) {
  try {
    ...
    // Generate unique filename without Cyrillic
    const uniqueFilename = generateUniqueFilename(slug, fileExt);
...
async function updateNewsImage(slug, imageUrl) {
  try {
    ...
    console.log(`   ✅ Updated news in database`);
    return true;
...
async function uploadAllRealImages() {
  ...
  console.log('📝 Using unique filenames to avoid Cyrillic issues\n');
  ...
  // Get all image files (exclude AI-generated and test images)
  const files = fs.readdirSync(REAL_IMAGES_DIR).filter(f => {
    ...
    return ['.jpg', '.jpeg', '.png'].includes(ext) &&
           ...
           !f.startsWith('image-') &&
           !f.startsWith('images-') &&
           !f.includes('featured') && // Skip featured images
           !f.match(/-\d\.jpeg$/); // Skip numbered variants like -1.jpeg, -2.jpeg
  });
  ...
  if (updated > 0) {
    ...
    console.log('✅ All news now have authentic photos from events!\n');
  } else {
    console.log('⚠️  No images were uploaded. Check errors above.\n');
  }
}
